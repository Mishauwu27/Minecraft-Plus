...
public class SculkSlabBlock extends SlabBlock implements SculkSpreadable {
    public static final MapCodec<SculkSlabBlock> CODEC = createCodec(SculkSlabBlock::new);
    ...
    @Override
    public MapCodec<? extends SculkSlabBlock> getCodec() { return CODEC; }
    ...
    private BlockState getExtraBlockState(WorldAccess world, BlockPos pos, Random random, boolean allowShrieker) {
        ...
        return blockState.contains(Properties.WATERLOGGED) && !world.getFluidState(pos).isEmpty()
                ? blockState.with(Properties.WATERLOGGED, Boolean.TRUE)
                : blockState;
    }
